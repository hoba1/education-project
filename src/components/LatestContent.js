...
export default function LatestContent(){
    ...
    const ShowLatestContent = ( contentName , content) => {
        return(
            <>
                <div className={`${contentName}-section mt-5 pt-3 pb-5`}>
                    <div className="container">
                        ...
                        <div className="row row-cols-1 row-cols-md-3 g-4 mt-5 mb-5">
                            {content.map((item) => {
                                if(content.indexOf(item) >= (content.length - 3)){               
                                    return(
                                        <Fade className="col col-md-6 col-lg-4 mb-4">
                                            <div className="card h-100 overflow-auto">
                                                {contentName === "lectures" && <img src={item.thumbnailUrl} className="card-img-top h-75" alt="..."/>}
                                                ...
                                                <div className="card-footer d-flex justify-content-between align-items-center" style={{padding: '0.5rem 0', margin: '0 16px'}}>
                                                    ...
                                                    <Link to={`/${contentName}/${item.id}`} className="btn text-light">open</Link>
                                                </div>
                                            </div>
                                        </Fade>
                                    )
                            ...
                            <Link to={`/${contentName}`} type="button" className="btn btn-outline-primary outline-btn fs-5 d-block mx-auto mt-5">Show More</Link>
                        </div>
                    </div>
                </div>
            </>
    ...
    return(
        <>
            {ShowLatestContent( 'lectures', lectures)}
            {ShowLatestContent( 'exams' , exams)}
        </>
    )
}