        ...
        <div className="container">
          ...
          <div className="row row-cols-1 row-cols-md-3 g-4 mt-5 mb-5">
            {props.content.map((item) => {
              if (
                props.latest
                  ? props.content.indexOf(item) >= props.content.length - 3
                  : props.content.indexOf(item) < limit
              ) {
                return (
                  <Fade className="col col-md-6 col-lg-4 mb-4">
                    <div className="card h-100 overflow-auto">
                      {props.contentName === "lectures" && (
                        <img
                      ...
                      <div className="card-body h-50">
                        <h5 className="card-title pb-2 text-center fs-3 fw-bold mb-3">
                          {item.title}
                        </h5>
                      ...
                      <div
                        ...
                        style={{ padding: "0.5rem 0", margin: "0 16px" }}
                      >
                        <small>{item.uploadTime}</small>
                        <div className="containerOfButtons d-flex gap-2">
                          <Link
                            to={`/${props.contentName}/${item.id}`}
                          ...
                          {props.control && (
                            <Link
                              to={`/data-${props.contentName}/${item.id}`}
                              className="btn text-light"
                            >
                              Get Full Data
                            </Link>
          ...
          {props.latest ? (
            <Link
              to={`/${props.contentName}`}
              type="button"
              className="btn btn-outline-primary outline-btn fs-5 d-block mx-auto mt-5 w-50"
            >
              Show More
            </Link>
          ) : (
